...
template<typename T> T shift(const uint8_t *buff, unsigned int no_bytes, udaq::common::bytes::Endianess endian)
{
    ...
    if (endian == udaq::common::bytes::Endianess::LittleEndian)
        for(unsigned int i = no_bytes-1; i >= 0; --i )
        {
            result <<= 8;
            result |= (T)buff[i];
        }
...
{
    double r;
    std::copy(buff, buff + sizeof(double), reinterpret_cast<char*>(&r));
...
template<typename T>
T udaq::common::bytes::swap_endian(const T val, typename std::enable_if<std::is_arithmetic<T>::value, std::nullptr_t>::type)
{
    // Taken from
