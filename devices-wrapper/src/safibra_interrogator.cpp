...
void* safibra_create_client(safibra_error_cb_t* erro_cb, safibra_connected_cb* client_connected_cb,
	...
	return new udaq::devices::safibra::SigprogServer([erro_cb](const std::string& msg) {(*erro_cb)(msg.c_str(), msg.length());},
		...
		[client_disconnected_cb]() {(*client_disconnected_cb)(); },
		[started_listening_cb]() {(*started_listening_cb)(); },
		[data_available_cb](std::vector<udaq::devices::safibra::SensorReadout> in) {
			...
			for (const auto& in_readout : in)
			{
				...
				data[0].readouts = new double[data[0].length];
